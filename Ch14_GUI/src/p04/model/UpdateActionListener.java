...
public class UpdateActionListener implements ActionListener{
	...
	public void actionPerformed(ActionEvent e) {
		...
		model.setValueAt(text1.getText(), row, 0);//해당 행의 첫번째 칼럼수정
		if(text2.getText().length()>0)
		model.setValueAt(text2.getText(), row, 1);//해당 행의 두번째 칼럼수정
		if(text3.getText().length()>0)
		model.setValueAt(text3.getText(), row, 2);//해당 행의 세번째 칼럼수정
	}
