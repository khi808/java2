...
/**
 *  클래스 파일(클래스명.java)에는 public클래스는 하나만 존재함
 *  public이 아닌 클래스는 여러개 존재함
 *  상속 -
 *  	부모와 자식이 같은 변수이면 자식의 변수를 사용함(부모의 값이 가려짐)
 *  	자식에서 동일한 부모의 변수를 사용하려면 super.변수명
 *  	자식의 변수에서는 this.가 생략됨.
 *  	자식클래스에서 부모의 클래스를 재정의하는것을 override 라고함.
 *  	override 된 메소드는 override 된 내용으로 실행됨.
 *
 * */
public class Super1 {
...
// 자식클래스 (B2) extends 부모클래스 (B1) // extends = 확장
class B2 extends B1 {
	// 자식클래스에서 필드를 재정의하면 부모필드의 값이 가려짐.
	int a=10; //B2의 필드
	void print() { // 메소드 재정의 - Override
		super.print();// 부모메소드의 print()메소드를 호출
		System.out.println("----------------------");
		System.out.println("자식 메소드, a ="+a);
		System.out.println("부모의 필드 a="+super.a);
	}
}
